...
| `phoenix`, `phoenix_live_view`, `phoenix_ecto` | core Hex | `lib/canary_web/router.ex`, `/dashboard` LiveView, `lib/canary_web/problem_details.ex`. Router pipeline scopes (`:scope_ingest`/`:scope_read`/`:scope_admin`) must survive any Phoenix router macro changes. |
