...
## Oracle
- [ ] Given a registered service, when `bin/canary dogfood value --service <name> --json` runs, then it returns coverage verdict, health state, error and incident counts, active remediation claim, recent annotations, recent telemetry events, witness or synthetic verification status, and one next action.
- [ ] Given `linejam` and `chrondle` are used as pilots, then the receipts distinguish a reference integration from an integration with current work, and include the last verified outcome or an explicit absence.
- [ ] Given a registry entry references completed backlog items such as `035` or `038`, then strict dogfood fails with a repair action that updates the registry language instead of preserving stale next actions.
- [ ] Given `bin/canary doctor --json` runs, then it summarizes dogfood value coverage with counts for covered, stale, blocked, partial, and value-unproven services.
...
2. Attach claim, annotation, event, incident, target/monitor, and integration evidence into the receipt.
