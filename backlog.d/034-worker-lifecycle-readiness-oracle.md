...
## Oracle
- [ ] `/readyz`, metrics, or a narrow read/admin endpoint reports one health snapshot for each lifecycle worker: started/stopped, last successful pass, failure count, and last error class without exposing secrets.
- [ ] Webhook delivery, target probe, monitor overdue, retention prune, and TLS scan worker tests assert that panics or runtime errors increment visible counters rather than disappearing into a catch-unwind boundary.
- [ ] The production image smoke in `dagger/src/index.ts` verifies `/healthz`, `/readyz`, and at least one worker-backed health signal or worker-health response.
- [ ] The implementation keeps worker internals behind deep modules; route handlers translate already-computed health snapshots rather than probing worker state ad hoc.
- [ ] `./bin/validate --fast` is green, and `./bin/validate` exercises the production image smoke.
